        ...
        if (stationRepository.count() > 0) {
            ...
            if (!bornesLyon.isEmpty()) {
                ...
                for (Station station : bornesLyon) {
                    ...
                    var exceptions = stationExceptionRepository.findByStationId(station.getId());
                    if (!exceptions.isEmpty()) {
                        stationExceptionRepository.deleteAll(exceptions);
                    }
                    var pictures = pictureRepository.findByStationId(station.getId());
                    if (!pictures.isEmpty()) {
